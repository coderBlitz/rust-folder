//! Cyclic redundancy check 32-bit (CRC-32)
//!
...
//!  * 0x04C11DB7 (CRC-32)
//!  * 0x1EDC6F41 (CRC-32C)
//!  * 0x42F0E1EBA9EA3693 (CRC-64-ECMA)
//!  * 0x1B (CRC-64-ISO)
...
/// TODO: Make generic and implement for all default integer types
const fn crc32u8(poly: u32, init: u32, data: u8) -> u32 {
	let mut rem: u32 = init ^ ((data as u32) << 24); // Shift data up to align with front of polynomial

	// Using while loop so function can be const
	let mut i = 0;
	while i < 8 {
...
///
/// TODO: Make generic and implement for all default integer types
fn crc32_tab(init: u32, data: &[u8]) -> u32 {
...
fn main() {
	//const P_r: u32 = 0xEDB8_8320;
	const P: u32 = 0x04C1_1DB7;

	let dat = [0x01, 0x02];

	let one = crc32u8(P, 0x0, dat[0]);
	println!("one = 0x{:08X}", one);
	assert_eq!(P, one);

	let won = crc32u8(P, 0xFFFF_FFFF, dat[0]);
	println!("Real one = 0x{:08X}", won);

	let tab_one = crc32_tab(0x0, &dat);
	println!("tab_one = 0x{tab_one:08X} | 0x{:08X}", tab_one ^ 0xFFFF_FFFF);

	let tab_two = crc32_tab(0xFFFF_FFFF, &[0x1,0x2,0x3,0x4]);
	println!("tab_two = 0x{:08X} | 0x{:08X}", tab_two, tab_two ^ 0xFFFF_FFFF);

	// Pre-computed table of stuff
	let byte_table = &mut Vec::with_capacity(256);
	for i in 0..=255 {
		byte_table.push(crc32u8(P, 0x0, i));
	}

	let out_bytes = byte_table
		.iter()
		.map(|x| format!("0x{:04X}_{:04X}", (x >> 16) & 0xFFFF, x & 0xFFFF))
		.collect::<Vec<String>>()
		.join(",");

	let out_file = &mut fs::File::create("/tmp/table.csv").expect("Could not open output file.");
	out_file.write(out_bytes.as_bytes()).expect("Error writing data");
}
